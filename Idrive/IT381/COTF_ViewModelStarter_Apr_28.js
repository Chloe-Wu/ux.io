...
function calcIt(){
	...
	var Principle = parseFloat($('#P').val());
	var TaxRate = parseFloat($('#T').val());
	Principle = parseFloat(Principle*(1+TaxRate)).toFixed(2);
	var Rate = (($('#R').val())/12)/100;
	Amount = Principle*(Rate*Math.pow(1+Rate,TotalPaymentsT))/(Math.pow(1+Rate,TotalPayments)-1);
	
	$('#Answer').html('With sales tax the '+currentModel+ " costs $ "+Principle+' so you can take it home today for only $'+Amount.toFixed(2)+' a month for'+TotalPayments+' months.Payments total $'+(Amount*TotalPayments).toFixed(2)+'.' );
